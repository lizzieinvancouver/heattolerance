...
#' * Phenological models for grape varieties for projections work 
#' BRIN model as in García de Cortazar-Atauri et al. 2009
#' Chuine model - chilling units as in Chuine et al. 2000
#' Wang & Engel model as in Cuccia et al. 2014
#'
...
## work in progress, models need to be checked against Iñaki's models in Excel!!

#rm(list=ls())

#' Chuine model Unichill. Unimodal and symetrical function. 2 parts - compute the function and get date
#'   After Cstar date (t1) is obtained, WE function is applied to obtain budbrake date
#' Part 1 - Fxchuine
...
#' 
#' Part 1b - SmoothUtah # alternative to Fxchuine, seems to work better
...
#GetCstardate<-function(Cstar,Tavg,result.chilling){
#  day1<-which(result.chilling[,2]>=Cstar)[1]
#  return(list(Cstardate=Tavg[day1,3:4],Cstardate=day1))
#}
...
#' @param Xa - observed date 
#' @param Xb - predicted date
#' @return RMSE 
...
RMSE <- function(Xa,Xb){
  ...
  ## Test with dummy dataset to test if the pipeline is operative
  #' Read summary climate and varieties data: csv files from excel file in:
  #' C:/.../GitHub/vin/climatefuture/models/analyses/inputs
  #' filenames: Database_v4_metdata.csv; pheno.results.per.variety.csv
  #' @param climate.data - data from METv4 in amber data file  
  #' @param variety.data - PHENOv4 pheno.results.per.variety.csv
  #' @param variety.parameters - fixed parameters in WE excel (a,b,c,Tmin,Tmax,Topt,C*,F*bb,F*Fl,F*Ver) ## WRONG FILE, need real param
  #' @return  Results.phenology - data frame (variety, site, year, BBdate, Fldate, Verdate)
  #' 
  #climate.data,variety.data,variety.parameters
  #25; variety: 2; year: 1"
...
# Modification of the function to work with Davis data 
yearly.phenology.dates.davis<-function(climate.data,variety.data,variety.parameters,
                                       UseBestparam,chillingfx=c("Chuine","Utah")){
  #variety.data<-varieties.davis
  #climate.data<-climate.data.local.Davis
  ## format/handle input data
  list.varieties<-as.character(unique(variety.data$Variety))
  site.var<-variety.data[,1:2]
  list.site.var<-site.var[!duplicated(site.var),]
  list.site.year<-climate.data[!duplicated(climate.data[,c(1,3)]),c(1,3)]
  #list.varieties[6]<-"Cabernet-Sauvignon"
      ...
      if(length(which(list.site.var[,1]==Variety & list.site.var[,2]==Site))!=0){
        ...
        for(i in 1:length(years.vector)){
          ...
          if(which(years.vector.met==Year.target)>1){
            Year.previous<-years.vector.met[which(years.vector.met==Year.target)-1]
            temps.avgs<-temps.avg[which(temps.avg$Year==Year.previous | temps.avg$Year==Year.target),]
            parameters<-variety.parameters[which(variety.parameters[,2]==Variety),2:23]
            
            if(UseBestparam==F & sum(is.na(parameters))==0){
              ## 2) get Budburst date (based on Chuine model) 
              if(chillingfx=="Chuine"){
                Chillunits<-Fxchuine(parameters$a,parameters$b,parameters$c,temps.avgs,Year.previous,Year.target,hemisphere)
              }
            ...
            if(UseBestparam==T & sum(is.na(parameters))==0 & !-9999.0%in%temps.avgs[,5]){
              ...
              ## 4) get BB, Flowering and Veraison dates 
              DateBB<-GetBBdate(parameters$Fstar_BB,temps.avgs,ActionunitsBB,DateCstar)
              Flow.date<-GetFlowdate(parameters$Fstar_BB_Fl,temps.avgs,ActionunitsFl,DateBB)
              if(!is.na(Flow.date$Flowdate.julian)){
                Verais.date<-GetVeraisdate(parameters$Fstar_Fl_Ver,temps.avgs,ActionunitsVer,Flow.date)
...
pheno.seq.fx<-function(x,variety){
  #x<-mat.avgs[1000,]
  Tm1=variety$Tm1
  Topt=variety$Topt
  Tn2=variety$Tn2
...
## Combined function to be applied to very large matrices (from raster datasets)
pheno.seq.fx.current<-function(x,variety,hem){
  #x<-Avg.temp
  Tm1=variety$Tm1
  Topt=variety$Topt
  Tn2=variety$Tn2
  ...
  Tavg=x
  #index.year<-which(period==Year.pheno)
  Year.previous<-Year.prev
  Year.target<-Year.pheno
  
  ## start with Smoothed-UTAH
  if(hem=="N"){Hemisphere="NORTH"}else{Hemisphere="SOUTH"}
